...
window.multiSelect = multiSelect;  // sobrescribe MM en su propia página
...
function funnelInmo() {
  return {
    ...
    async loadFilterOptions() {
      const r = await fetch(`/api/funnel/inmo?action=filters&fecha_desde=${this.fechaDesde}&fecha_hasta=${this.fechaHasta}`);
      this.filtersOptions = await r.json();
    },
    async loadEtapas() {
      const r = await fetch("/api/funnel/inmo?action=etapas");
    ...
    async refreshMetas() {
      ...
      try {
        ...
        const [realRes, kpiRes] = await Promise.all([
          fetch(`/api/funnel/inmo?action=metas-real&${buildQS(params)}`).then(r => r.json()),
          fetch(`/api/funnel/inmo?action=metas-kpi-tendencias&${buildQS(params)}`).then(r => r.json()),
        ]);
    ...
    async refreshNegocios() {
      ...
      try {
        ...
        const r = await fetch(`/api/funnel/inmo?action=negocios&${buildQS(params)}`);
    ...
    async refreshVolumen() {
      ...
      try {
        const r = await fetch(`/api/funnel/inmo?action=volumen&${buildQS(filterParamsInmo())}`);
    ...
    async refreshShareCat() {
      ...
      try {
        const r = await fetch(`/api/funnel/inmo?action=share-cat&${buildQS(filterParamsInmo())}`);
        const data = await r.json();
    ...
    async refreshConvTime() {
      ...
      try {
        ...
        const r = await fetch(`/api/funnel/inmo?action=conv-time&${buildQS(params)}`);
