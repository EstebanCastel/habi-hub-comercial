...
function convSeller() {
  return {
    ...
    async loadCycles() {
      ...
      this.ciclo = current ? current.ciclo : (this.cicloOptions[0]?.ciclo || 0);
      this.updatePeriodoLabel();
    },

    async loadData() {
      const r = await fetch("/api/conversion/seller?action=data");
    ...
    updatePeriodoLabel() {
      const p = this.cicloOptions.find(c => c.ciclo === this.ciclo);
      if (p) this.periodoLabel = `📅 Asignación: ${p.asig_start} → ${p.asig_end} · Cierre: ${p.cierre_start} → ${p.cierre_end}`;
    },
    ...
    exportCsv() {
      ...
      a.href = url;
      a.download = `conv_seller_${this.producto}_c${this.ciclo}_${new Date().toISOString().slice(0,10)}.csv`;
